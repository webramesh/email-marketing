...
export class PlatformAnalyticsService {
  ...
  async getTenantsOverview(
    ...
    const tenantsOverview: TenantOverview[] = tenants.map(tenant => {
      ...
      const totalEmailsSent = tenant.campaigns.reduce((sum, campaign) => sum + campaign.totalSent, 0)
  ...
  async getRevenueMetrics(): Promise<RevenueMetrics> {
    ...
    const averageRevenuePerUser = totalActiveUsers > 0 
      ? (totalRevenue._sum.amount || 0) / totalActiveUsers 
      : 0
    ...
    const revenueByPlanData = revenueByPlan.map(plan => ({
      planName: plan.name,
      revenue: plan.subscriptions.reduce((sum, sub) => 
        sum + sub.tenant.payments.reduce((paySum, payment) => paySum + payment.amount, 0), 0
      ),
